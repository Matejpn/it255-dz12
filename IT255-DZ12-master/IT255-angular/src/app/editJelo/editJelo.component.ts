...
export class EditKonkretnoJelo  { 
  ...
  ngOnInit() {
    this.route.params.subscribe((params: Params) => {
      ...
      this.http.get('http://localhost:8888/php/citajSamoJednoJelo.php?id='+id,{headers:headers}).map(res => res.json()).share()
        .subscribe(data => {
            this.data = data.data;
    ...
    onEdit(): void {
  	  this.route.params.subscribe((params: Params) => {
	      ...
	      headers.append('Content-Type', 'application/x-www-form-urlencoded');
	      headers.append("token",localStorage.getItem("token"));
	      this.http.post('http://localhost:8888/php/izmeniJelo.php', data, { headers: headers })
	      .map(res => res)
